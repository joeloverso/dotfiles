...
# Requests is a Python library for making HTTP requests. Used to make API calls to OpenRouter.
# Requests is a Python library for making HTTP requests. Used to make API calls to OpenRouter.
import requests
...
# Constants
MAX_QUESTION_COUNT = 30  # Maximum number of questions that can be graded
        ...
        try:
            ...
            for i, assignment in enumerate(assignments):
                ...
                if canvas_type == "new_quiz":
                    # Grade with new quiz workflow
                    needs_grading = True  # Loop control flag
                    while needs_grading:
                        needs_grading = check_if_student_has_submission(page)
                        if not needs_grading:  # If student does not need grading, grading for this assignment is finished
                            break
                    ...
                    # Use the "Assignment" grading workflow
...
def validate_simple_input(user_input: Any, y: str, n: str) -> str:
    """
    To validate simple user input such as yes (y) or no (n)
    Or Mr.NiceGuy (n) or Mr.BytheBook (b)
    ...
    while True:
        ...
        if user_input == y:
            ...
            user_input = input(f"Enter {y} or {n}:  ")


def validate_question_input(
    user_input: Any, min: int = 1, max: int = MAX_QUESTION_COUNT
) -> int:
    """
    Validates that a question number input is within the allowed range.
    Returns the validated integer question number.
    """
    while True:
        try:
            question_num = int(user_input)
            if min <= question_num <= max:
                return question_num
            else:
                print(
                    f"    Invalid input. Please enter a number between {min} and {max}."
                )
        except ValueError:
            print(
                f"    Invalid input. Please enter an integer between {min} and {max}."
            )
        user_input = input("Enter question number: ")
...
def sign_in(page, user_inputs):
    ...
    try:
        ...
        # Wait for successful login URL with a timeout
        try:
            page.wait_for_url("https://ocps.instructure.com/?login_success=1", timeout=30000)
            print("Successfully logged in!")
            humanizer(8, 10)
        except Exception as e:
            print(f"An error occurred during sign-in\n Error details: {str(e)}")
            print("Please manually enter MFA code.\nResume script after successful sign-in.")
            page.pause()
...
def get_urls():
    """
    Get URLs and grading preferences for Canvas assignments.

    This function prompts the user to:
    1. Enter assignment URLs one at a time
    2. Specify if each is a New Quiz or regular assignment
    3. For New Quizzes, select AI grading preferences
    4. Choose whether to add additional assignments

    Returns:
        list: A list of dictionaries, where each dictionary contains:
            - url (str): The assignment URL
            - type (str): Either "new_quiz" or "assignment"
            - ai_mode (dict): For new quizzes only, contains AI grading preferences:
                - mode (str): "null", "niceguy", or "book"
                - target_q (int): Target question number for niceguy mode
    """
    ...
    while True:
        ...
        is_new_quiz = validate_simple_input(is_new_quiz, "y", "n")
        # If user said yes, Canvas type is New Quiz otherwise it is "Assignment"
        canvas_type = "new_quiz" if is_new_quiz == "y" else "assignment"
        ...
        assignments.append(assignment)  # Add assignment to list

        # Ask if user wants to add another assignment
        add_url = input("Do you have another assignment to grade? (y/n): ")
        add_url = validate_simple_input(add_url, "y", "n")  # Validate add_url
        if add_url == "y":  # If user has another assignment to grade
            url = input("Enter the URL of the assignment to grade: ")
        elif add_url == "n":
            break
...
def get_ai_grading_choice():
    ...
    choice = validate_simple_input(choice, "y", "n")
    if choice == "n":
        ai_mode["mode"] = "null"
        return ai_mode  # Return the null mode
    else:
        choice = input(
            "How do you want to grade with AI? Press (n) for Mr. NiceGuy or (b) for Mr.By the Book book. Press n or b and hit enter: "
        )
        choice = validate_simple_input(choice, "n", "b")
        if choice == "n":
            ai_mode["mode"] = "niceguy"
            print("Mr.NiceGuy will grade all questions LENIENTLY.")
            target_q = input(
                "Would you like him to target a specific question for him to provide feedback on? (y/n): "
            )
            target_q = validate_simple_input(target_q, "y", "n")
            if target_q == "y":
                target_q = input(
                    "Which question number would you like Mr.NiceGuy to target? Please enter the number: "
                )
                ai_mode["target_q"] = target_q
                return ai_mode
            else:  # User selected no target question
                return ai_mode
        else:  # User chose b for Mr.BytheBook
            ai_mode["mode"] = "book"
            return ai_mode


# For New Quiz questions
def mr_nice_guy_grade_answer(question: str, answer: str) -> bool:
    """
    Uses google/gemini‑flash‑2.5 preview on OpenRouter to judge correctness with Mr. NiceGuy's lenient grading style.
    The model must answer ONLY 'true' or 'false'.
    Returns True if the answer is correct, False if incorrect after triple-checking.
    """
    headers = {
        # Open Router API key
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "You are Mr.NiceGuy, a lenient teacher whose students are low level readers."
                    "You are grading classwork for completion credit not 'correctness'."
                    "Return just the word 'true' if the student answer is expected from a 8th grader with very low reading ability."
                    "Stimulus relevant for knowing the correct answer to the assignment has been lost."
                    "Simply determine if the answer is a good try."
                    "When in doubt give credit even if the answer is incorrect or even only somewhat reasonable."
    ...
    # Try up to 3 times if we get a False response
    for attempt in range(3):
        try:
            ...
            # If we get a True response, return True immediately
            if re.match(r"^true\b", reply):
                return True

            # If we get a False response, try again unless this is our last attempt
            elif re.match(r"^false\b", reply):
                if attempt < 2:  # If not our last attempt
                    print(f"⚠️ AI returned False on attempt {attempt + 1}, retrying...")
                    continue
                else:
                    print("⚠️ AI returned False on all three attempts")
                    return False
            else:
            ...
            print(f"Error during AI grading attempt {attempt + 1}: {e}")
            if attempt == 2:  # If this was our last attempt
                return True  # We'll be generous if all attempts fail
            continue

    return True  # Default to True if something goes wrong
...
def navigate_to_speedgrader(page, assignment_numb, current_assignment_url):
    ...
    try:
        ...
        page.wait_for_selector("#speedgrader_iframe", state="attached", timeout=20000)
...
def find_questions_need_grading(page):
    ...
    for i in range(1, MAX_QUESTION_COUNT):  # Limit to 30 questions for efficiency
        try:
...
# For new quiz
def find_student_answers(
    page, questions_found, grading_decisions, reload_counter, assignment
):
    ...
    try:
        ...
        # 5) Grade each answer based on AI mode
        for qnum, (q_text, a_text) in enumerate(
            zip(questions, student_answers), start=1
        ):
            if a_text == "(no answer)":
                grading_decisions[qnum] = "incorrect"
            elif "ai_mode" in assignment and assignment["ai_mode"]["mode"] != "null":
                # Use AI grading if enabled
                correct = mr_nice_guy_grade_answer(q_text, a_text)
                grading_decisions[qnum] = "correct" if correct else "incorrect"
            else:
                # No AI or no answer - give participation credit
    ...
    try:
        ...
        if question.count() > 0:
            ...
            for i in range(0, len(all_buttons), 2):
                if current_question < len(questions_found) and i + 1 < len(all_buttons):
...
def mark_incorrect(page, question_number, button_map):
    ...
    try:
        add_question_comment(page, question_number, comment_text="Lorem Ipsum")
    except Exception as e:
        print(f"Error adding comment to question {question_number}: {str(e)}")


def add_question_comment(page, question_number: int, comment_text: str) -> None:
    """
    Adds a comment to a specific question in the SpeedGrader.

    Stuff:
        page: The Playwright page object
        question_number: The question number to add the comment to
        comment_text: The text of the comment to add
    """
    try:
        print(f"Attempting to add comment to question {question_number}...")

        # Get the SpeedGrader iframe
        frame = page.frame_locator("#speedgrader_iframe")

        # First get the quiz content region
        quiz_region = frame.get_by_role("region", name="Quiz content")
        quiz_region.wait_for(state="visible", timeout=10000)

        # Click the question link to ensure it's loaded
        frame.get_by_role(
            "link", name=f"Question {question_number}", exact=True
        ).click()

        # Find the question wrapper within the quiz region
        question_wrapper = quiz_region.locator(
            f"div[data-automation='sdk-grading-result-wrapper']:has-text('Question {question_number}')"
        ).first
        question_wrapper.wait_for(state="visible", timeout=10000)

        # Try to find the comment button within the wrapper
        comment_button = None
        try:
            # First try: Look for a button with specific text within the wrapper
            comment_button = question_wrapper.locator(
                "button:has-text('Comment')"
            ).first
            if comment_button.count() == 0:
                # Second try: Look for any button that might be a comment button
                comment_button = question_wrapper.locator("button").first
        except Exception as e:
            print(f"Error finding comment button: {str(e)}")
            page.pause()  # Pause to inspect the question wrapper
            return

        if not comment_button or comment_button.count() == 0:
            print("Could not find comment button")
            page.pause()  # Pause for manual inspection
            return

        comment_button.click()

        # Wait for the rich text editor iframe to be available
        editor_frame = frame.locator(
            'iframe[title="Rich Text Area. Press OPTION+F8 for Rich Content Editor shortcuts."]'
        )
        editor_frame.wait_for(state="visible", timeout=10000)

        # Get the editor content area and fill it with the comment
        editor_content = editor_frame.frame_locator("iframe").get_by_role(
            "application", name="Rich Text Area."
        )
        editor_content.click()
        editor_content.fill(comment_text)

        # Click the Done button
        done_button = frame.get_by_role("button", name="Done")
        done_button.click()

        print(f"Successfully added comment to question {question_number}")

    except Exception as e:
        print(f"Error adding comment to question {question_number}: {str(e)}")
        print("Pausing for debugging...")
        page.pause()  # Pause for debugging if something goes wrong
...
def mark_questions(page, questions_found, grading_decisions):
    ...
    # page.goto("https://ocps.instructure.com/courses/2184230/gradebook/speed_grader?assignment_id=35828809&student_id=3668410")
    # page.locator("#speedgrader_iframe").content_frame.locator(".css-my9yfq > .css-1d8qg77-view--inlineBlock-baseButton").first.click()
    # page.locator("#speedgrader_iframe").content_frame.locator("iframe[title=\"Rich Text Area\\. Press OPTION\\+F8 for Rich Content Editor shortcuts\\.\"]").content_frame.get_by_role("application", name="Rich Text Area. Press OPTION+").click()
    # page.locator("#speedgrader_iframe").content_frame.locator("iframe[title=\"Rich Text Area\\. Press OPTION\\+F8 for Rich Content Editor shortcuts\\.\"]").content_frame.get_by_role("application", name="Rich Text Area. Press OPTION+").fill("Sample text.")
    # page.locator("#speedgrader_iframe").content_frame.get_by_role("button", name="Done").click()


def submit_student_grade(page):
    page.pause()  # For debugging purposes. Need to test code and capture comment button, comment textbox, comment send text, comment DONE button.
    ...
    try:
        ...
        if update_button.count() > 0:
            ...
            if not update_clicked:
                print("WARNING: Could not find any Update button to finalize grades.")
    ...
    try:
        ...
        if grade_out_of_element.count() > 0:
            ...
            if points_match:
                ...
                print("Could not extract points from text, prompting for manual entry")
