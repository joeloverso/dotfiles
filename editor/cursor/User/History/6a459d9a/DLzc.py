...
def main():
    ...
    def get_user_credentials():
        user_inputs = {}  # For passing user inputs to various functions
        print("Please enter your login credentials...")

        user_inputs["username"] = input("Enter your username: ")
        user_inputs["password"] = stdiomask.getpass(
            prompt="Enter your password: ", mask="*"
        )

        user_inputs["MFACode"] = input("Enter your MFA code: ")
        return user_inputs

    def random_wait(min_wait=1, max_wait=2):
    ...
    def get_urls():
        # Create a list to store assignments with their types
        assignments = []
        ...
        while True:
            ...
            # AI prompt for new quiz:
            if is_new_quiz:
                ai_mode, ai_single_q = get_ai_grading_choice()
            # Add to assignments list as a dictionary
            assignments.append({"url": url, "type": canvas_type})
    ...
    def ai_grade_answer(question: str, answer: str) -> bool:
        """
        ...
        try:
            ...
            try:
                ...
                while not ensure_answers_accessible(page):
                    ...
                    random_wait(2, 3)  # Wait between attempts
            except Exception as e:
    ...
    def find_student_answers(page, questions_found, grading_decisions, reload_counter):
        ...
        try:
            ...
            for qnum, (q_text, a_text) in enumerate(zip(questions, student_answers), start=1):
                should_use_ai = (
                    ai_mode == "all"
                    or (ai_mode == "one" and qnum == ai_single_q)
                )
    ...
    def get_grading_buttons(page, questions_found):
        ...
        try:
            ...
            if question.count() > 0:
                ...
                question.focus()
                question.click()
                random_wait(1, 2)  # Wait for buttons to become visible
    ...
    def does_student_need_grading(page):
        ...
        try:
            ...
            if no_submission_element.count() > 0:
                needs_grading = False
                print("Student does not need grading\nNo more students to grade.")
            else:
                needs_grading = True
                print("Student needs grading")

        except Exception as e:
            # If there's an error, log it and assume student needs grading
            print(f"Error checking if student needs grading: {str(e)}")
            needs_grading = True
            print("Assuming student needs grading due to error")
    ...
    def grade_assignment_workflow(page):
        ...
        try:
            ...
            needs_grading = True
            while needs_grading:
                needs_grading = does_student_need_grading(page)
                ...
                random_wait(2, 3)  # Allow page to load
    ...
    def grade_current_student(page, points_possible):
        try:
            ...
            if grade_input.count() > 0:
                ...
                random_wait(1, 2)  # Wait for input to register
            else:
            ...
            if submit_button.count() > 0:
                ...
                submit_button.click()
                random_wait(2, 3)  # Wait for submission to process
    ...
    # Welcome user and introduce the script
    welcome()
